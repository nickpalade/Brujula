# Consolidation — one brain, not two (proposal, Pepe; executed, Ceco)

**Update: `server/agent/*` + `verify-agent.js` are deleted.** Reached the same
conclusion independently while untangling the two SQLite stores, before
spotting this doc — `/api/*` is confirmed working end-to-end (store, hub
routes, live server boot all smoke-tested this session), so the deletion went
ahead without waiting for a GPU-laptop `verify:hub` run. If that run turns up
something `/api/*` was missing, `server/agent/*` is still in git history
(before this doc's update) to recover from.

**Original TL;DR recommendation: the `/api/*` stack (server/pipeline/* + routes/hub.js +
...
- ~~**Delete `server/agent/*` + `verify-agent.js`**~~ — done. The root `hub.db`
  (the second SQLite file) is retired along with it; `data/hub.db` is now the
  only store. Worth running `verify:hub` green on the GPU laptop regardless,
  as a confidence check rather than a gate.
